...
interface IdentityStepProps {
  firstName: string;
  lastName: string;
  onUpdate: (first: string, last: string) => void;
  onNext: () => void;
}

const IdentityStep: React.FC<IdentityStepProps> = ({ firstName, lastName, onUpdate, onNext }) => {
      ...
      <div className="flex flex-col gap-6 w-full flex-1">
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            value={firstName}
            onChange={(e) => onUpdate(e.target.value, lastName)}
            placeholder="Jane"
          />
        </div>
